...
class DynamicBicycleModel:
    ...
    def step_jnp(self, 
        ...
        # st = time.time()
        # next_psi = jnp.arctan2(jnp.sin(next_psi), jnp.cos(next_psi))
        alpha_f = steer - jnp.arctan2(self.batch_LF * batch_omega + batch_vy, jnp.maximum(batch_vx,batch_vx*.0+.5))
        alpha_r = jnp.arctan2(self.batch_LR * batch_omega - batch_vy, jnp.maximum(batch_vx,batch_vx*.0+.5))
    ...
    def step_(self, 
        ...
        # next_psi = jnp.arctan2(jnp.sin(next_psi), jnp.cos(next_psi))
        alpha_f = steer - jnp.arctan2(self.params.LF * batch_omega + batch_vy, jnp.maximum(batch_vx,batch_vx*.0+.5))
        alpha_r = jnp.arctan2(self.params.LR * batch_omega - batch_vy, jnp.maximum(batch_vx,batch_vx*.0+.5))
        # print(alpha_f, alpha_r)
        # print(batch_vx, batch_vy)
        F_rx = throttle
        # print("Frx", F_rx, "alpha f", alpha_f, "alpha r", alpha_r, 'vx', batch_vx, 'vy', batch_vy, 'omega', batch_omega, 'steer', steer)
...
def dbm_dxdt(
    ...
    # st = time.time()
    # next_psi = jnp.arctan2(jnp.sin(next_psi), jnp.cos(next_psi))
    # alpha_f = steer - jnp.arctan2(batch_LF * batch_omega + batch_vy, jnp.maximum(batch_vx,batch_vx*.0+.5))
    alpha_f = steer - jnp.arctan((batch_LF * batch_omega + batch_vy) / jnp.maximum(batch_vx,batch_vx*.0+.5))
    # alpha_r = jnp.arctan2(batch_LR * batch_omega - batch_vy, jnp.maximum(batch_vx,batch_vx*.0+.5))
    alpha_r = jnp.arctan((batch_LR * batch_omega - batch_vy) / jnp.maximum(batch_vx,batch_vx*.0+.5))
