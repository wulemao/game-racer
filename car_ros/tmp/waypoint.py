...
class WaypointGeneratorFromMsg:
    def __init__(self, msg, dt: float = 0.1, H: int = 10, vehicle_params: dict = None):
        """
        Args:
            msg: Message object containing the 'wpnts' list
            dt: Prediction time step
            H: Prediction horizon steps
            vehicle_params: Dictionary of vehicle parameters {'Lf': float, 'Lr': float}
        """
        if vehicle_params is None:
            # If not provided, use common default values (inferred from original code)
            vehicle_params = {'Lf': 0.158, 'Lr': 0.171}
        ...
        # -----------------------------------------------------------
        # 1. Parse message and construct self.path [s, x, y, v]
        # -----------------------------------------------------------
        # Extract data lists
        s_list = [x.s_m for x in msg.wpnts]
        x_list = [x.x_m for x in msg.wpnts]
        y_list = [x.y_m for x in msg.wpnts]
        v_list = [x.vx_mps for x in msg.wpnts]
        ...
        # Combine into (N, 4) matrix; column order strictly matches custom_fn requirements
        # Column 0: s (theta)
        # Column 1: x
        # Column 2: y
        # Column 3: v
        self.path = np.column_stack((s_arr, x_arr, y_arr, v_arr))
        
        # Convert to JAX array (required by generate function)
        self.path = jnp.array(self.path)

        # -----------------------------------------------------------
        # 2. Calculate left/right boundaries (using d_left/d_right and psi from msg)
        # -----------------------------------------------------------
        psi_arr = np.array([x.psi_rad for x in msg.wpnts])
        d_left_arr = np.array([x.d_left for x in msg.wpnts])
        d_right_arr = np.array([x.d_right for x in msg.wpnts])

        # Left boundary: Extend along the normal to the left of the heading angle
        # Assumption: Frenet coordinate definition where left is the positive normal direction
        # left_x = x - d_left * sin(psi)
        # left_y = y + d_left * cos(psi)
        lx = x_arr - d_left_arr * np.sin(psi_arr)
        ly = y_arr + d_left_arr * np.cos(psi_arr)
        self.left_boundary = np.column_stack((lx, ly))

        # Right boundary: Extend along the normal to the right of the heading angle
        # right_x = x + d_right * sin(psi)
        # right_y = y - d_right * cos(psi)
        rx = x_arr + d_right_arr * np.sin(psi_arr)
        ry = y_arr - d_right_arr * np.cos(psi_arr)
        self.right_boundary = np.column_stack((rx, ry))

        # -----------------------------------------------------------
        # 3. Pre-computation - Keep consistent with original logic
        # -----------------------------------------------------------
        self.scale_val = 6.5 * (float(vehicle_params['Lf']) + float(vehicle_params['Lr']))
        
        # Generate discrete points for searching based on max s value
        # Note: self.path[-1, 0] must be the total length of the path (max_s)
        max_s = self.path[-1, 0]
        self.waypoint_t_list = jnp.arange(0, max_s, 0.1 * self.scale_val)

        # Vectorized pre-computation of discrete point states to accelerate runtime search
        # Use custom_fn for interpolation here
        self.waypoint_list = jnp.array([self.fn(t, self.path)[0] for t in self.waypoint_t_list])
        self.waypoint_list_np = np.array(self.waypoint_list)

    def line_point_distance(self, px, py, x1, y1, x2, y2):
        """Keep original helper function unchanged"""
        dx, dy = x2 - x1, y2 - y1
        denom = dx*dx + dy*dy
        return jnp.where(denom == 0, 0.0, ((px - x1) * dx + (py - y1) * dy) / denom)

    def generate(self, obs: jnp.ndarray, dt=-1, mu_factor=1., body_speed=1.) -> jnp.ndarray:
        """Keep original core logic unchanged"""
        dt = self.dt if dt < 0 else dt
        pos2d = obs[:2]
