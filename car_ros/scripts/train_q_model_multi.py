...
data_path = os.path.join(DATA_DIR, f"{args.data_name}{suffix}.pkl")
...
if args.model_name != "none":
    ...
    if not os.path.isabs(model_path):
        model_path = os.path.join(Q_MODELS_DIR, model_path)
...
Y = data[:, 1:, 0] - data[:, :-1, 0]
Y_ = data[:, 1:, 1] - data[:, :-1, 1]
Y__ = data[:, 1:, 2] - data[:, :-1, 2]
...
def save_model(m, name):
    path = os.path.join(Q_MODELS_DIR, f"{name}{suffix}.pth")
